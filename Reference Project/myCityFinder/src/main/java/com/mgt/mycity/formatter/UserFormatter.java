/*
 * (c) Copyright 2005-2012 JAXIO, www.jaxio.com
 * Source code generated by Celerio, a Jaxio product
 * Want to use Celerio within your company? email us at info@jaxio.com
 * Follow us on twitter: @springfuse
 * Template pack-mvc-3:src/main/java/formatter/Formatter.e.vm.java
 */
package com.mgt.mycity.formatter;

import java.text.ParseException;
import java.util.Locale;

import com.mgt.mycity.domain.User;
import com.mgt.mycity.service.UserService;
import com.mgt.mycity.formatter.support.DiscoverableFormatter;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

/**
 * {@link Formatter} for {@link User} that is picked by automatically by {@link CustomFormattingConversionServiceFactory}
 * 
 * @see Formatter
 * @see CustomFormattingConversionServiceFactory
 */
@Component
public class UserFormatter implements DiscoverableFormatter<User> {

    @Autowired
    private UserService userService;

    @Override
    public Class<User> getTarget() {
        return User.class;
    }

    @Override
    public String print(User user, Locale locale) {
        if (user == null) {
            return "";
        }

        StringBuilder sb = new StringBuilder();
        // make a nice display string here...

        if (user.getEmailaddress() != null) {
            sb.append(user.getEmailaddress()).append(" ");
        }

        if (user.getPassword() != null) {
            sb.append(user.getPassword()).append(" ");
        }

        if (user.getEnabled() != null) {
            sb.append(user.getEnabled()).append(" ");
        }

        if (sb.length() > 0) {
            return sb.toString();
        } else if (user.isPrimaryKeySet()) {
            return user.getPrimaryKey().toString();
        } else {
            return user.toString();
        }
    }

    @Override
    public User parse(String text, Locale locale) throws ParseException {
        if (text == null || text.isEmpty()) {
            return new User();
        }
        User user = userService.getByPrimaryKey(new Integer(text));
        return user != null ? user : new User();
    }
}
