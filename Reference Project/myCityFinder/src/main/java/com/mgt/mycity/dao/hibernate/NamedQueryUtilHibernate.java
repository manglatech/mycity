...
import com.mgt.mycity.dao.support.OrderBy;
...
public class NamedQueryUtilHibernate implements NamedQueryUtil {
    ...
    private static final Logger log = Logger.getLogger(NamedQueryUtilHibernate.class);
    ...
    public Object objectByNamedQuery(SearchTemplate searchTemplate, Object model) {
        if (searchTemplate == null || !searchTemplate.hasNamedQuery()) {
            log.warn("Invalid search template provided");
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("objectNamedQuery " + searchTemplate.toString());
            }
            ...
            if (log.isDebugEnabled()) {
                log.debug(searchTemplate.getNamedQuery() + " returned a " + (ret == null ? "null" : ret.getClass())
                        + " object");
                if (ret instanceof Number) {
                    log.debug(searchTemplate.getNamedQuery() + " returned a number with value : " + ((Number) ret));
                }
            }

            return ret;
        } catch (Exception e) {
            log.error("Could not load object by named query", e);
    ...
    public List<Object> findByNamedQuery(SearchTemplate searchTemplate, Object model) {
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("findByNamedQuery " + searchTemplate.toString());
            }
            ...
            if (c != null && log.isDebugEnabled()) {
                log.debug(searchTemplate.getNamedQuery() + " returned a List of size: " + c.size());
            }

            return c;
        } catch (Exception e) {
            log.error("Could not load List by named query", e);
            throw new RuntimeException(e);
        }
    }
